...
class AuctionRepositoryTest {
    ...
    @Test
    @DisplayName("마감되지 않은 모든 경매 내역 조회 & 조회수 기준 내림차순 정렬")
    void findAllByIsDoneOrderByViewsDesc() {
        //given
        testAuction1.setView(10);
        testAuction2.setView(9);
        testAuction3.setView(8);
        testAuction4.setView(7);
        testAuction5.setView(6);
        testAuction6.setView(5);
        testAuction7.setView(4);

        Pageable pageable = PageRequest.of(0, 5);
        LocalDateTime fakeNow = LocalDateTime.parse("2000년 01월 10일 00시 00분 00초",
                DateTimeFormatter.ofPattern("yyyy년 MM월 dd일 HH시 mm분 ss초"));

        testAuction1.setIsDone(fakeNow);
        testAuction2.setIsDone(fakeNow);

        List<Auction> expectedResult = new ArrayList<>(List.of(testAuction3, testAuction4, testAuction5, testAuction6, testAuction7));
        expectedResult.sort(Comparator.comparing(Auction::getViews).reversed());

        //when
        Page<Auction> testResult = auctionRepository.findAllByIsDoneOrderByViewsDesc(false, pageable);
        List<Auction> testResultList = testResult.getContent();
    ...
    @Test
    @DisplayName("마감되지 않은 모든 경매 내역 조회 & 마감 기한 기준 오름차순")
    void findAllByIsDoneOrderByDeadlineAsc() {
        //when
    ...
    @Test
    @DisplayName("마감되지 않은 모든 경매 내역 조회 & 조회수 별 내림차순")
    void findAllByIsDoneOrderByBidSizeDesc() {
    }
}